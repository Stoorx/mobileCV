...
void rgb_to_hsv(uint8_t* rgb, uint8_t* hsv, int num_pixels) {
    for(int i=0; i<num_pixels; ++i, rgb+=3, hsv+=3) {
            std::swap(rgb[0], rgb[2]);
        int maxc = max(rgb[0], max(rgb[0 + 1], rgb[0 + 2]));
        int minc = min(rgb[0], min(rgb[0 + 1], rgb[0 + 2]));
        int delta = maxc - minc;
        ...
        if (delta == 0) {
            ...
            h *= 30;
            h /= delta;
        } else if (rgb[0 + 1] == maxc) {
            h = rgb[0 + 2] - rgb[0] + 2 * delta;
            h *= 30;
            h /= delta;
        } else {
            h = rgb[0] - rgb[0 + 1] + 4 * delta;
            h *= 30;
            h /= delta;
        }
        hsv[0] = h;
        ...
        if (maxc == 0)
            ...
            hsv[1] = 255 * delta / maxc;
        
        hsv[2] = maxc;
    }
}

void rgb_to_hsv_neon(uint8_t* rgb, uint8_t* hsv, int num_pixels)
{
    num_pixels /= 8;
    uint8x8x3_t result;
    auto t1_neon = chrono::high_resolution_clock::now();
    ...
    for(int i=0; i<num_pixels; ++i, rgb+=8*3, hsv+=8*3) {
        ...
        for (size_t j = 0; j < 3; ++j) {
            ...
            uint16x8_t sub = vsubl_u8(src.val[i1], src.val[i2]);
            if (j == 0) {
                auto tmp = vclt_u8(src.val[1], src.val[2]);
            ...
            intermediate_result = vaddq_u16(intermediate_result, sub);
...
int main(int argc,char** argv)
{
    ...
    auto t1_neon = chrono::high_resolution_clock::now();
    rgb_to_hsv_neon(rgb_arr, hsv_arr, num_pixels);
    auto t2_neon = chrono::high_resolution_clock::now();
    cvtColor(hsv_image, rgb_image, COLOR_HSV2BGR);
    auto duration_neon = chrono::duration_cast<chrono::microseconds>(t2_neon-t1_neon).count();
    cout << "rgb_to_gray" << endl;
    cout << duration_neon << " us" << endl;
